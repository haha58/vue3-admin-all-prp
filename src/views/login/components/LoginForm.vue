<template>
	<el-form ref="loginFormRef" :model="loginForm" :rules="loginRules" size="large">
		<el-form-item prop="username">
			<el-input v-model="loginForm.username" placeholder="用户名：wenyan">
				<template #prefix>
		...
		<el-form-item prop="password">
			<el-input type="password" v-model="loginForm.password" placeholder="密码：wenyan" show-password autocomplete="new-password">
				<template #prefix>
...
const loginForm = reactive({ username: "wenyan", password: "wenyan" });
const onLogin = formEl => {
	...
	formEl.validate(async valid => {
		...
		try {
			...
			ElNotification({
				title: data.name,
				message: "欢迎登录 Vue3-Template-AdminPro",
				type: "success",
				duration: 3000
			});
		} finally {
